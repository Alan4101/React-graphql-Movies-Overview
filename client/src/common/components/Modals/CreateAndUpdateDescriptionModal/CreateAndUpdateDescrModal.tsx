    ...
    <MovieModal isOpen={isOpenModal} toggleModal={toggleModal}>
      <Grid container>
        <Grid item xs={12} justifyContent='center'>
          <Typography sx={{ padding: '15px 0', textAlign: 'center' }} variant='h4'>
        ...
        <Grid container item md={12} xs={12}>
          <StyledForm>
            <MovieTextField
              ...
              rows={8}
            />
            <Grid container flexDirection='row' gap={2} justifyContent='flex-end'>
              <MovieButton variant='outlined' onClick={resetForm}>
                {t('content.button.cancel')}
              </MovieButton>
              <MovieButton variant='contained' onClick={(e: any) => handleSubmit(e)}>
                {isUpdate ? t('content.button.update') : t('content.button.save')}
              </MovieButton>
            </Grid>
          </StyledForm>
        </Grid>
